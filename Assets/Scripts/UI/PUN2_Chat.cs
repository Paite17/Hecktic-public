// Copied the code from here: https://www.sharpcoderblog.com/blog/pun-2-adding-room-chat
// so big ups to them
...
public class PUN2_Chat : MonoBehaviourPun
{
    ...
    void SendChat(Player sender, string message)
    {
        ...
        // modification that adds a "(SPEC)" prefix to the username if the user sending the chat is a spectator
        if (sender.CustomProperties.TryGetValue("IsSpectator", out object isSpectator))
        {
            if (!(bool)isSpectator)
            {
    ...
    // functions for getting the player controller so that i can turn off movement while chatting

    private PlayerMovement GetLocalPlayerController(PhotonView view)
    {
        PlayerMovement[] allPlr = FindObjectsOfType<PlayerMovement>();

        foreach (var current in allPlr)
        {
            if (current.photonView.Owner == PhotonNetwork.LocalPlayer)
            {
                return current;
            }
        }
    ...
    public void GetLocalPlayerForChat()
    {
        thisPlayer = GetLocalPlayerController(photonView);
    }
}
